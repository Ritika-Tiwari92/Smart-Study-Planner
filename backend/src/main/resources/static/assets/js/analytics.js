...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function normalizeSubject(subject) {
    ...
    return {
      ...
      progress: safeNumber(
        firstAvailable(
          subject,
          ["progress", "completionPercentage", "coverage"],
          0,
        ),
        0,
      ),
      difficulty: firstAvailable(
        subject,
  ...
  function extractSubjectId(obj) {
    ...
    if (obj.subject && typeof obj.subject === "object")
      return String(obj.subject.id || obj.subject.subjectId || "");
    return "";
  }
  ...
  function normalizeStatus(value) {
    ...
    if (["in progress", "progress", "active", "ongoing"].includes(status))
      return "IN_PROGRESS";
  ...
  function normalizeTask(task) {
    const dueDate = firstAvailable(
      task,
      ["dueDate", "date", "createdAt", "updatedAt"],
      "",
    );
    const status = normalizeStatus(
      firstAvailable(task, ["status", "taskStatus"], "PENDING"),
    );
    ...
    return {
      ...
      subjectName:
        firstAvailable(task, ["subjectName"], "") ||
        extractSubjectName(task.subject),
  ...
  function normalizeRevision(revision) {
    return {
      ...
      subjectName:
        firstAvailable(revision, ["subject", "subjectName"], "") ||
        extractSubjectName(revision.subject),
      date: dateToYmd(
        firstAvailable(revision, ["revisionDate", "date", "createdAt"], ""),
      ),
      status: normalizeStatus(
        firstAvailable(revision, ["status", "priority"], "PENDING"),
      ),
  ...
  function normalizePlan(plan) {
    return {
      ...
      title: firstAvailable(
        plan,
        ["title", "topic", "planTitle", "name"],
        "Study Plan",
      ),
      subjectName:
        firstAvailable(plan, ["subject", "subjectName"], "") ||
        extractSubjectName(plan.subject),
      date: dateToYmd(
        firstAvailable(plan, ["date", "planDate", "createdAt"], ""),
      ),
  ...
  function normalizeTest(test) {
    const rawScore = firstAvailable(
      test,
      ["score", "percentage", "marks", "obtainedScore"],
      null,
    );
    const score =
    ...
    return {
      ...
      subjectName:
        firstAvailable(test, ["subject", "subjectName"], "") ||
        extractSubjectName(test.subject),
      date: dateToYmd(
        firstAvailable(test, ["testDate", "date", "createdAt"], ""),
      ),
      score: Number.isFinite(score) ? score : null,
      status: Number.isFinite(score)
        ? "COMPLETED"
        : normalizeStatus(
            firstAvailable(test, ["status", "adminStatus"], "PENDING"),
          ),
    };
  }

  function normalizePomodoro(session) {
    const dateValue = firstAvailable(
      session,
      ["sessionDate", "startedAt", "createdAt", "startTime"],
      "",
    );
    const minutes = safeNumber(
      firstAvailable(
        session,
        ["actualDurationMinutes", "focusMinutes", "actualMinutes"],
        0,
      ),
      0,
    );
    ...
    return {
      ...
      subjectName: firstAvailable(
        session,
        ["linkedSubjectName", "subjectName", "topic"],
        "Focus Session",
      ),
  ...
  function calculateMetrics(data) {
    ...
    const completedPomodoroCount = useBackendPomodoro
      ...
      : completedPomodoro.length;

    const focusEfficiency = useBackendPomodoro
      ? clamp(safeNumber(backendPomodoro.productivityScore, 0), 0, 100)
      : completedPomodoro.length
        ? clamp(
            Math.round((focusMinutes / completedPomodoro.length / 25) * 100),
            0,
            100,
          )
        : 0;
    ...
    const weeklyConsistency =
      getSelectedRange() === "This Week" &&
      Number.isFinite(Number(weeklyActiveDays))
    ...
    return {
      ...
      averageTestScore,
      completedSessions: completedPomodoroCount,
      weeklyConsistency,
      taskCompletion,
      revisionCompletion,
      planCompletion,
  ...
  function renderScoreOverview(metrics) {
    ...
    setScoreDescription(
      ...
      metrics.completedSessions
        ? `${metrics.focusMinutes} minutes from ${metrics.completedSessions} Pomodoro sessions`
        : "No Pomodoro focus data yet",
    );
  }
  ...
  function buildSubjectInsights(data) {
    ...
    const insights = names.map((name) => {
      ...
      const taskScore = tasks.length
        ? percent(tasks.filter(isCompleted).length, tasks.length)
        : NaN;
      const revisionScore = revisions.length
        ? percent(revisions.filter(isCompleted).length, revisions.length)
        : NaN;
      ...
      const planScore = plans.length
        ? percent(plans.filter(isCompleted).length, plans.length)
        : NaN;
      ...
      const focusScore = focusMinutes
        ? clamp(
            Math.round(
              (focusMinutes / Math.max(25, pomodoro.length * 25)) * 100,
            ),
            0,
            100,
          )
  ...
  function renderSubjectInsights(insights) {
    ...
    els.subjectInsightList.innerHTML = insights
      .slice(0, 6)
      .map((item, index) => {
        const isStrongest = index === 0 && insights.length > 1;
        const isWeakest =
          weakest &&
          item.subjectName === weakest.subjectName &&
          weakest.finalScore < strongest.finalScore;
        const pctClass =
          item.finalScore >= 70
            ? "pct-high"
            : item.finalScore >= 45
