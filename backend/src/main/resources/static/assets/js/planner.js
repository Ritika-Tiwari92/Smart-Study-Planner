...
const PLANNER_API_BASE_URL = "/api/plans";
const SUBJECTS_API_BASE_URL = "/api/subjects";
...
function getAiPlanMeta(plan) {
  ...
  return {
    ...
    estimatedHoursLabel: estimatedHours
      ? normalizeHoursLabel(estimatedHours)
      : "",
  };
}
...
function getPlanIconClass(subject, title, description = "") {
  ...
  if (text.includes("java") || text.includes("coding"))
    return "fa-solid fa-laptop-code";
  if (text.includes("dbms") || text.includes("sql"))
    return "fa-solid fa-database";
  if (text.includes("network")) return "fa-solid fa-globe";
  if (text.includes("operating") || text.includes("os"))
...
function updateSummaryCards(plans) {
  ...
  const completedToday = todayPlans.filter(
    (plan) => (plan.status || "").toLowerCase() === "done",
  );
  const completedWeekly = weeklyPlans.filter(
    (plan) => (plan.status || "").toLowerCase() === "done",
  );
  ...
  if (completedTodayCount)
    completedTodayCount.textContent = String(completedToday.length).padStart(
      2,
      "0",
    );
...
function applyPlanFilters() {
  const search = planSearchInput
    ? planSearchInput.value.toLowerCase().trim()
    : "";
  ...
  let filtered = [...allPlans]
    .filter((plan) => {
      const aiKeywords = isAiSyllabusPlan(plan)
        ? "ai syllabus plan generated topic"
        : "";
...
function renderFocusTips(plans) {
  ...
  const pending = plans.filter(
    (plan) => (plan.status || "").toLowerCase() === "pending",
  );
  const done = plans.filter(
    (plan) => (plan.status || "").toLowerCase() === "done",
  );
  const subjects = [
    ...new Set(weeklyPlans.map((plan) => plan.subject).filter(Boolean)),
  ];
  const upcoming = plans
    .filter((plan) => plan.date > today)
    .sort(comparePlansAsc);
  ...
  if (todayPlans.length > 0) {
    const names = [
      ...new Set(todayPlans.map((plan) => plan.subject).filter(Boolean)),
    ];
    ...
    tips.push(
      "No sessions planned for today. Add a study plan to stay on track.",
    );
  }

  if (aiPlans.length > 0) {
    tips.push(
      `${aiPlans.length} AI syllabus session(s) are scheduled from your uploaded syllabus.`,
    );
  }

  if (pending.length > 0) {
    tips.push(
      `You have ${pending.length} pending plan(s). Try to complete at least one session today.`,
    );
  }
  ...
  if (upcoming.length > 0) {
    ...
    tips.push(
      `Next session: "${next.title}" on ${label} at ${formatTime(next.time)}. Be ready!`,
    );
  }
...
function fillPlanFormForEdit(sessionItem) {
  ...
  planTitleInput.value =
    sessionItem.querySelector(".session-info h4")?.textContent.trim() || "";
...
async function loadSubjectOptions(selectedValue = "") {
  ...
  try {
    ...
    const alreadyExists = [...planSubjectInput.options].some(
