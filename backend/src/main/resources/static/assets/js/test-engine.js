...
const examTotalQuestionsValue = document.getElementById(
  "examTotalQuestionsValue",
);

const engineQuestionTypeTop = document.getElementById("engineQuestionTypeTop");
const engineCurrentQuestionTop = document.getElementById(
  "engineCurrentQuestionTop",
);

const engineSectionName = document.getElementById("engineSectionName");
const engineQuestionProgressChip = document.getElementById(
  "engineQuestionProgressChip",
);
...
const sidebarCandidateSubject = document.getElementById(
  "sidebarCandidateSubject",
);
const sidebarExamName = document.getElementById("sidebarExamName");
const sidebarCurrentQuestion = document.getElementById(
...
const engineClearResponseBtn = document.getElementById(
  "engineClearResponseBtn",
);
const engineSaveMarkReviewBtn = document.getElementById(
  "engineSaveMarkReviewBtn",
);
const engineMarkReviewNextBtn = document.getElementById(
  "engineMarkReviewNextBtn",
);
...
const legendNotAnsweredCount = document.getElementById(
  "legendNotAnsweredCount",
);
...
const legendAnsweredMarkedCount = document.getElementById(
  "legendAnsweredMarkedCount",
);

const engineQuestionPalette = document.getElementById("engineQuestionPalette");
const enginePaletteCollapseBtn = document.getElementById(
  "enginePaletteCollapseBtn",
);
...
const cancelSubmitSummaryBtn = document.getElementById(
  "cancelSubmitSummaryBtn",
);
...
function buildStartTestApiUrl(testId) {
  return `${TESTS_API_URL}/${encodeURIComponent(testId)}/start`;
}

function buildSubmitAttemptApiUrl(attemptId) {
  return `${TESTS_API_URL}/attempts/${encodeURIComponent(attemptId)}/submit`;
}
...
function saveTheoryAnswerSilently() {
  ...
  const value = normalizeAnswerValue(
    engineTheoryAnswer ? engineTheoryAnswer.value : "",
  );
  if (value) {
...
function renderQuestionMeta() {
  ...
  if (engineQuestionText) {
    engineQuestionText.textContent =
...
function renderQuestionInputArea() {
  ...
  if (engineOptionsList) {
    ...
    engineOptionsList.innerHTML = options
      .map((option) => {
...
function renderPalette() {
  ...
  engineQuestionPalette.innerHTML = examSession.questions
    .map((question, index) => {
      const status = getQuestionStatus(question.id);
...
function isSummaryModalOpen() {
  return (
    submitSummaryOverlay && !submitSummaryOverlay.classList.contains("hidden")
  );
}
...
function openSubmitSummaryModal() {
  ...
  if (summaryRemainingTime)
    summaryRemainingTime.textContent = examRemainingTime
      ? examRemainingTime.textContent
      : "00:00:00";
...
async function submitExam(forceSubmit = false) {
  ...
  try {
    ...
    const result = await fetchJson(
      buildSubmitAttemptApiUrl(examSession.attemptId),
      {
        method: "POST",
        headers: {
...
function bindLanguageSelector() {
  examLanguageSelect?.addEventListener("change", () => {
    ...
    if (selected === "Hindi") {
      showEngineToast(
        "Hindi UI is ready, but translated question content must come from backend.",
        "info",
      );
    }
  });
